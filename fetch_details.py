# -*- coding: utf-8 -*-
"""Fetch full article content via NewsAPI real URLs + extract images."""
...
def extract_article_and_images(html):
    ...
    if not paras:
        body = tree.find('body')
        if body is not None:
            return body.text_content().strip()[:3000], images
        return '', images
    return '\n\n'.join(paras[:15]), images

def llm_translate(text):
    if not text or len(text) < 50:
        return text
    prompt = '请将以下英文新闻内容翻译成流畅自然的中文。\n\n要求：\n- 保持原文段落结构\n- 语言专业流畅，新闻风格\n- 保留关键数据和引用\n- 直接输出中文翻译，不加前缀\n\n英文原文：\n' + text[:4000] + '\n\n中文翻译：'
    body = {
        "model": "deepseek-v4-pro",
        "messages": [
            {"role": "system", "content": "你是一个专业的中英新闻翻译。"},
...
def main():
    ...
    while i < len(lines):
        ...
        if m:
            article = {
                'title': m.group(1), 'url': m.group(2), 'region': m.group(3),
            ...
            while j < len(lines):
                cmt = lines[j].strip()
                if cmt.startswith('<!-- IMG:'):
                    article['image_url'] = cmt[8:-3].strip()
                elif cmt.startswith('<!-- CNT:'):
                    article['content'] = cmt[8:-3].strip()
                elif cmt.startswith('<!-- SRC:'):
                    article['source_name'] = cmt[8:-3].strip()
    ...
    print('\nFetching full articles (' + str(total) + ' items)...\n')
    detail_map = {}
    for idx, article in enumerate(items[:total]):
        ...
        print('  Source: ' + source_name + ', URL: ' + url[:80])
        ...
        if html:
            ...
            if article_text:
                ...
                print('  Fetched: ' + str(len(en_text)) + ' chars text, ' + str(len(imgs)) + ' images')
            else:
                print('  No text extracted from article')
            if imgs:
                images = imgs
        else:
            print('  Could not fetch article (paywall/blocked)')

        if not en_text:
            en_text = newsapi_content if newsapi_content else desc
        if newsapi_img and not images:
            images = [newsapi_img]
        elif newsapi_img:
            images.insert(0, newsapi_img)

        if len(en_text) > 50:
            cn_text = llm_translate(en_text)
            if cn_text and len(cn_text) > 50:
                print('  Translated: ' + str(len(cn_text)) + ' chars CN')
            else:
        ...
        page = gen_page(title, url, rgn, pubdate, cn_text, images, en_text[:2000] if en_text else '')
