# -*- coding: utf-8 -*-
"""NewsAPI + article fetch + clean images + LLM Chinese content."""
...
AD_PATTERNS = ['/ad/', '/ads/', '/banner/', '/leaderboard/', '/promo/', '/sponsor',
               'b2b3_', 'button336', 'leaderboard', 'btn_', '_ad_', 'advert']
...
def extract_article_and_images(html):
    """Extract article text and REAL content images, filtering ads."""
    ...
    # Collect images with aggressive ad filtering
    images = []
    for img in tree.xpath('//img'):
        ...
        if w and h and w.isdigit() and h.isdigit():
            wi, hi = int(w), int(h)
            # Filter banners (wide but short) and tiny images
            if wi > 300 and hi < 80: continue  # banner ad
            if wi < 150 or hi < 150: continue  # too small

        # Filter by URL patterns
        src_lower = src.lower()
        if any(p in src_lower for p in AD_PATTERNS):
            continue
        if any(s in src_lower for s in ['icon', 'logo', 'avatar', 'pixel', '1x1', 'spacer', 'tracking', 'analytics']):
            continue
        if 'google' in src_lower and 'googleusercontent' not in src_lower:
            continue
    ...
    # Remove noise tags for text extraction
...
def gen_page(title, source_url, region, pubdate, cn_text, images, en_text=''):
    ...
    # Image section - same format as old version
    img_html = ''
    ...
    # English original (collapsible) - same format as old version
    en_section = ''
    if en_text and len(en_text.strip()) > 50:
        en_section = '<details style="margin-top:20px;padding:14px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;"><summary style="font-size:13px;color:#64748b;cursor:pointer;">英文原文摘要</summary><p style="font-size:14px;color:#64748b;line-height:1.8;margin:10px 0 0;white-space:pre-wrap;">' + escape(en_text[:2000]) + '</p></details>\n'
...
def main():
    ...
    for line in content.split('\n'):
        m = re.match(r'- \[(.*?)\]\(([^)]+)\) \| (\w+)(?: \| (.*?) \| (.*))?', line)
        if m:
            items.append({'title': m.group(1), 'url': m.group(2), 'region': m.group(3),
                          'pubdate': m.group(4) or '', 'desc': m.group(5) or '',
                          'image_url': '', 'content': '', 'source_name': ''})

    # Parse embedded metadata
    for i, line in enumerate(content.split('\n')):
        if line.startswith('<!-- IMG:') and i > 0:
            idx = len([x for x in items if content.split('\n').index(line) > content.split('\n').index('- [' + x['title'][:20])])
        pass  # Simplified - already have items
    ...
    for idx, article in enumerate(items[:total]):
        ...
        # Extract image_url, content, source_name from HTML comments following this article
        # Since the metadata parsing is complex, let me use a simpler approach
        # Read the report file and extract metadata for this article
        with open(os.path.join(SRC, fs[0]), encoding='utf-8-sig') as f:
        ...
        for rl in report_lines:
            if title[:30] in rl and rl.startswith('- ['):
                found_article = True
                continue
            if found_article and rl.startswith('<!-- IMG:'):
                newsapi_img = rl[8:-3].strip()
            elif found_article and rl.startswith('<!-- CNT:'):
                newsapi_content = rl[8:-3].strip()
            elif found_article and rl.startswith('<!-- SRC:'):
                source_name = rl[8:-3].strip()
            elif found_article and (rl.startswith('- [') or rl.startswith('# ') or rl.startswith('---')):
                break
        ...
        # Use NewsAPI image as fallback NOT as primary
        if not images and newsapi_img:
            images = [newsapi_img]
